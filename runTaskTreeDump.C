int runTaskTreeDump (TString runMode="",TString analysisMode="",TString inputName="",TString inputOptions="",TString softVersions="",TString analysisOptions="",TString taskOptions=""){
  ...
  SetupAnalysis(runMode,analysisMode,inputName,inputOptions,softVersions,analysisOptions, "libPWGmuon.so AliAnalysisTaskUpsilonTreeDump.cxx AddTaskUpsilonTreeDump.C",". $ALICE_ROOT/include $ALICE_PHYSICS/include","UpsilonTaskOutputTreeDump");
  ...
  AliAnalysisTaskUpsilonTreeDump* taskMy = AddTaskUpsilonTreeDump(isMC, "first");
